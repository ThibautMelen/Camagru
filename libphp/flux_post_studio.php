...
$addDomPost = $bdd->prepare('SELECT pseudo, avatar, picture, like_nb FROM member INNER JOIN post ON member.id = post.member_id WHERE member.id = ? ORDER BY post.id DESC LIMIT '.$nbPost.'');
